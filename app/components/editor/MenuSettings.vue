...
function handleChildDragOver(event: DragEvent, parentIndex: number, childIndex: number) {
  ...
  if (draggingChildIndex.value.parentIndex !== parentIndex) return // 只允许在同父级内拖拽
...
function addMenuItem() {
  newMenuLabel.value = ''
  addingAsChild.value = false
  addChildParentIndex.value = null
...
function confirmAddMenuItem() {
  ...
  if (!label) {
    showMessage('请输入菜单标签', 'error')
    return
  }
  
  const newItem: ActionMenuItem = {
    label: label,
    messages: []
  }
...
function openEditNameDialog(index: number) {
  ...
  editingNameIsChild.value = false
  editingNameParentIndex.value = null
  const item = menuItems.value[index]
  if (item) {
    newMenuName.value = item.label
  }
...
function openEditChildNameDialog(parentIdx: number, childIdx: number) {
  editingNameIndex.value = childIdx
  editingNameIsChild.value = true
  editingNameParentIndex.value = parentIdx
  const parent = menuItems.value[parentIdx] as any
  if ('child' in parent && parent.child && parent.child[childIdx]) {
    newMenuName.value = parent.child[childIdx].label
  }
  showEditNameDialog.value = true
}
...
function confirmEditName() {
  ...
  if (editingNameIsChild.value && editingNameParentIndex.value !== null) {
    ...
    if ('child' in parent && parent.child && parent.child[editingNameIndex.value!]) {
      parent.child[editingNameIndex.value!].label = name
    }
  } else if (editingNameIndex.value !== null) {
    // 编辑主菜单名称
    const item = menuItems.value[editingNameIndex.value]
    if (item) {
      item.label = name
    }
  }
...
function addChildItem(parentIdx: number) {
  newMenuLabel.value = ''
  addingAsChild.value = true
  addChildParentIndex.value = parentIdx
  showAddDialog.value = true
}
...
function editChildItem(parentIdx: number, childIdx: number) {
  editingIndex.value = childIdx
  isAddingChild.value = true
  parentIndex.value = parentIdx
  showEditor.value = true
}
...
function deleteChildItem(parentIdx: number, childIdx: number) {
  if (confirm('确定要删除这个子菜单项吗？')) {
    ...
    if (parent && 'child' in parent && parent.child) {
      parent.child.splice(childIdx, 1)
      autoSave()
    }
  }
}
...
function handleSaveMenuItem(messages: any[]) {
  if (isAddingChild.value && parentIndex.value !== null) {
    ...
    if (editingIndex.value !== null) {
      ...
      parent.child[editingIndex.value] = {
        label: existingChild.label,
        messages: messages
      }
    } else {
      // 添加新子项
    ...
    if (editingIndex.value !== null) {
      ...
      if (existingItem) {
        menuItems.value[editingIndex.value] = {
...
function handleUpdateKeyword(newKeyword: string) {
  ...
  const item = menuItems.value[editingIndex.value]
  if (!item) return
  ...
  // 更新菜单项的 label
  item.label = newKeyword.trim()
  autoSave()
  showMessage('菜单名称已更新', 'success')
}

// 判断是否有子菜单
function hasChildren(item: ActionMenuItem): boolean {
  return 'child' in item && Array.isArray(item.child)
}

// 获取子菜单数量
function getChildCount(item: ActionMenuItem): number {
  if ('child' in item && item.child) {
...
<template>
  <article class="panel-card">
      ...
      >
        <div class="flex justify-between items-start md:items-center mb-3 gap-2">
          <div class="flex items-center gap-2 flex-1 min-w-0">
            ...
            <span v-else class="text-xs text-[#4caf50] bg-[rgba(76,175,80,0.1)] px-2 py-0.5 rounded-full font-medium whitespace-nowrap">
              {{ (item as any).messages?.length || 0 }} 条消息
            </span>
          </div>
          <div class="flex gap-1.5 md:gap-2 shrink-0">
            <!-- 有子菜单的项：只能编辑子菜单和删除，不能编辑发送消息 -->
            <template v-if="hasChildren(item)">
              <button 
                class="py-1 px-2 md:py-1.5 md:px-3 border-none rounded-md text-xs md:text-[13px] cursor-pointer transition-all duration-200 font-medium bg-[rgba(76,175,80,0.1)] text-[#4caf50] hover:bg-[rgba(76,175,80,0.2)]" 
              ...
              <button 
                class="py-1 px-2 md:py-1.5 md:px-3 border-none rounded-md text-xs md:text-[13px] cursor-pointer transition-all duration-200 font-medium bg-[rgba(156,39,176,0.1)] text-[#9c27b0] hover:bg-[rgba(156,39,176,0.2)]" 
                @click="convertToFlat(index)"
                title="转换为扁平菜单"
              >
                转为扁平
              </button>
            ...
            <template v-else>
              ...
              <button 
                class="py-1 px-2 md:py-1.5 md:px-3 border-none rounded-md text-xs md:text-[13px] cursor-pointer transition-all duration-200 font-medium bg-[rgba(33,150,243,0.1)] text-[#2196f3] hover:bg-[rgba(33,150,243,0.2)]" 
                @click="convertToGroup(index)"
                title="转换为分组菜单"
              >
                转为分组
              </button>
        ...
        <div v-if="hasChildren(item) && 'child' in item && item.child && item.child.length > 0" class="mt-3 pt-3 border-t border-dashed border-[#e8ddd3] dark:border-[#2b2d30] flex flex-col gap-2">
          <div 
            ...
            :key="childIndex"
            draggable="true"
            @dragstart="handleChildDragStart(index, Number(childIndex))"
            @dragover="handleChildDragOver($event, index, Number(childIndex))"
    ...
    <DialogMessageList
      ...
      :title="isAddingChild ? '编辑子菜单项' : (editingIndex !== null ? menuItems[editingIndex]?.label || '编辑菜单项' : '添加菜单项')"
      keyword-label="菜单名称"
      :show-loading-time="true"
      :editable="!isAddingChild && editingIndex !== null"
      @update:messages="handleSaveMenuItem"
