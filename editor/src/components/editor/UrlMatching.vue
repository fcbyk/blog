...
function addItem() {
  ...
  if (!key) { showMessage('请输入参数名', 'error'); return }
  if (items.value[key]) { showMessage('该参数已存在', 'error'); return }
  items.value[key] = []
  newItem.value = ''
  autoSave()
}
...
function updateItemMessages(messages: (MessageConfigWithRole | { role?: undefined })[]) {
  ...
  const messagesWithRole = messages.map(msg => ({ ...msg, role: msg.role || 'bot' })) as MessageConfigWithRole[]
...
function updateItemKeyword(newKeyword: string) {
  ...
  if (!oldKey) return
  if (!newKeyword) { showMessage('查询字符串不能为空', 'error'); return }
  if (newKeyword === oldKey) return
  if (items.value[newKeyword]) { showMessage('该参数已存在', 'error'); return }
